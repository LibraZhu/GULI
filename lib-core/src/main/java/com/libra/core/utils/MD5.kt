...
object MD5 {
     ...
     * string md5
     */
    fun encode(source: String, upper: Boolean = false): String {
        val btyes = source.toByteArray()
        val encodedBytes = sDigest!!.digest(btyes)

        return hexString(encodedBytes, upper)
    }
     ...
     * 文件 md5
     */
    fun encodeFile(inputFile: String, upper: Boolean = false): String? {
        // 缓冲区大小（这个可以抽出一个参数）
        ...
        try {
            ...
            // 同样，把字节数组转换成字符串
            return hexString(resultByteArray, upper)
        } catch (e: NoSuchAlgorithmException) {
            return null
        } catch (e: IOException) {
            e.printStackTrace()
    ...
    fun hexString(source: ByteArray?, upper: Boolean = false): String {
